...
inner join student_payment r on (q.studentpayment_id=r.id)) w where  pay_day BETWEEN date_format('".$start_date."','%Y-%m-%d') and date_format('".$end_date."','%Y-%m-%d') and type='product' and item_name='Matriculas'  group by studentpayment_id) ba
        ...
        left join courses h on (e.course_id=h.id) GROUP BY id) w where w.course_name<>'' group by course_name order by course_name";
        return $this->db->query($sql)->result();
    }
    public function get_student_not_report($start_date,$end_date){
        $sql="select * ,(h.course_fee*monpay)mon_total
        from (select a.status, a.name as student_name,a.id,a.phone,c.name as class_name,d.name as course_name,d.course_fee,d.mulct, e.session_year, (date_format('".$end_date."','%m')-date_format('".$start_date."','%m')+1-ifnull(ww.monpay,'0'))monpay,ifnull(ww.last_mon,' ')last_mon,ifnull(ww.last_pay,' ')last_pay from students a
        ...
        ) ww on (a.id=ww.student_id)) h where h.monpay<>0 and h.status=1 order by class_name";
        return $this->db->query($sql)->result();
    }
    ...
    public function get_student_course_report($school_id = null, $academic_year_id = null, $course_id = null, $period = null,$class_id = null,$section_id = null){       
        $sql = "select c.name as student_name, e.name as course_name, w.name as class_name, q.name as section_name, c.period  from (select * from student_payment_details where item_name='Inscrição') a
    ...
    public function get_admit_student_course_report($school_id = null, $academic_year_id = null, $course_id = null, $period = null,$class_id = null,$section_id = null){       
